...
class LocalStore {
  ...
  async createSession(userId) { const token = crypto.randomUUID(); this.data.sessions = this.data.sessions.filter((session) => new Date(session.expiresAt).getTime() > Date.now()); this.data.sessions.push({ token, userId, createdAt: now(), expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString() }); this.save(); return token; }
  async getSession(token) { const session = this.data.sessions.find((item) => item.token === token); return session && new Date(session.expiresAt).getTime() > Date.now() ? session : null; }
  async deleteSession(token) { this.data.sessions = this.data.sessions.filter((item) => item.token !== token); this.save(); }
...
async deleteSession(token) {
  ...
  async init() {
    ...
    this.memberships = this.db.collection("memberships");
    this.resources = this.db.collection("resources");
    this.assignments = this.db.collection("assignments");
    this.discussions = this.db.collection("discussions");
    ...
    await Promise.all([
        ...
        this.memberships.createIndex({ groupId: 1, userId: 1 }, { unique: true }),
        this.resources.createIndex({ groupId: 1, createdAt: -1 }),
        this.assignments.createIndex({ groupId: 1, dueDate: 1 }),
        this.chunks.createIndex({ groupId: 1, resourceId: 1 }),
        this.chunks.createIndex({ content: "text", resourceTitle: "text" })
    ]);
