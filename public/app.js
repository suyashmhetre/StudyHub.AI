...
function resourceCard(resource) {
  ...
  return `<article class="resource-card"><div class="resource-top"><div class="file-icon ${iconClass}">${escapeHtml(resource.type.slice(0, 4).toUpperCase())}</div><span class="resource-menu">•••</span></div><h3>${escapeHtml(resource.title)}</h3><p>${escapeHtml(resource.description || resource.fileName)}</p><div class="tag-row">${(resource.tags || []).slice(0, 3).map((tag) => `<span class="tag">${escapeHtml(tag)}</span>`).join('')}</div><div class="resource-footer"><span>${escapeHtml(resource.unit || 'General')}</span><span>↧ ${resource.downloads || 0}</span><a class="button button-secondary" href="/api/resources/${resource.id}/download">Download</a>${button('Remove', 'delete-resource', 'button-danger').replace('data-action="delete-resource"', `data-action="delete-resource" data-resource="${resource.id}" data-group="${resource.groupId}"`)}</div></article>`;
}
